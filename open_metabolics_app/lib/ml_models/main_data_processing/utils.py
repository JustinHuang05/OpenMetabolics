...
def processRawGait_model(data_array, start_ind, end_ind, weight, height, correction_url, cur_device, num_bins=30):
    """Process raw gait data for model input preparation and use a web service for artifact correction."""
    fs = 50
    gait_data = data_array[start_ind:end_ind, :]
    ...
    # Prepare payload for correction model API
    payload = json.dumps({
        "features": np.insert(model_input, 0, dur_stride).tolist()
    })
    headers = {"Content-Type": "application/json"}

    # Make a request to the correction model's web service
    response = requests.post(correction_url, data=payload, headers=headers)
    if response.status_code == 200:
        est_artifact = np.array(response.json().get("prediction")).flatten()
        model_input = model_input - est_artifact
    else:
        raise ValueError(f"Correction model API failed with status code: {response.status_code}, response: {response.text}")
...
def estimateMetabolics(model_url, gait_data, peak_index, weight, height, cur_basal, correction_url, stride_detect_window):
    """Estimate energy expenditure using model predictions based on processed gait data through a web service."""
    ee_all = []
    for i in range(len(peak_index) - 1):
        ...
        if (gait_stop_index - gait_start_index) <= stride_detect_window and (gait_stop_index - gait_start_index) > 35:
            ...
            model_input = processRawGait_model(gait_data, gait_start_index, gait_stop_index, weight, height, correction_url, None).tolist()
            
            # Prepare payload for the energy expenditure model API
            payload = json.dumps({"features": model_input})
            headers = {"Content-Type": "application/json"}

            # Make a request to the energy expenditure model's web service
            response = requests.post(model_url, data=payload, headers=headers)
            if response.status_code == 200:
                ee_est = response.json().get("prediction")[0]
                print(f"Bout {i + 1}: Predicted energy expenditure = {ee_est:.2f} kcal/min.")
                ee_all.append(ee_est)
            else:
                raise ValueError(f"Energy expenditure model API failed with status code: {response.status_code}, response: {response.text}")
        else:
            print(f"Bout {i + 1}: Insufficient data or invalid stride length, using basal metabolic rate.")
