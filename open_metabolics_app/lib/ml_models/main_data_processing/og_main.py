...
cur_basal = og_utils.basalEst(height, weight, age, gender, stand_aug_fact, kcalPerDay2Watt=0.048426)
...
active_idx = og_utils.get_active_phase(gyro_l2_norm_sp, sliding_win, gyro_norm_thres)
active_idx = og_utils.find_consecutive(active_idx)
...
for idx, cur_active_idx in enumerate(active_idx):
    ...
    prin_idx_pocket = og_utils.find_prin_axis(cur_pocket_gyro_rot_zx)
    ...
    gait_peaks = og_utils.peak_detect(prin_gyro_pocket)
    
    if len(gait_peaks) > 3:  # Consider it a bout if more than 3 peaks are detected
        gait_data_pocket = og_utils.segment_data(gait_peaks, cur_pocket_gyro_rot_zx, stride_detect_window)
        ...
        opt_rotm_y_pocket, theta_y = og_utils.get_rotate_y(avg_gait_data_pocket, prin_idx_pocket)
        ...
        if gyro_z_norm_pos <= gyro_z_norm_neg:
            opt_rotm_pocket = np.matmul(opt_rotm_pocket, og_utils.rotm_y(np.pi))
            theta_y += 180
            if theta_y > 180:
                theta_y -= 360
            cur_pocket_gyro_cal = np.matmul(cur_pocket_gyro, opt_rotm_pocket)
        ...
        gait_peaks = og_utils.peak_detect(cur_pocket_gyro_cal[:, -1])
